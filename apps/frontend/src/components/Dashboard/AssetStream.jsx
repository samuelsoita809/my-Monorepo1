...
export const AssetStream = ({ healthStatus, isLoading, products, LoadingScreen, onDelete, onEdit }) => (
    <section className="glass-panel rounded-3xl overflow-hidden border border-white/5 shadow-2xl w-full">
        ...
        <div className="p-2 md:p-6 bg-black/20 overflow-x-auto w-full">
            <Suspense fallback={<LoadingScreen />}>
                {isLoading ? <LoadingScreen /> : <InventoryTable items={products} onDelete={onDelete} onEdit={onEdit} />}
            </Suspense>
        </div>
    </section>
);
